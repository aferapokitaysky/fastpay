...
| `POST /v1/public/payment-intents/:id/loyalty-optin` (B3) | `{ phone }` → `200 { optedIn: true, visitCount, totalSpentKopecks }`. Доступно только когда intent уже `succeeded` — `409 PAYMENT_NOT_SUCCEEDED` иначе. Апсертит `guest_profiles` по `(organizationId, hash(phone))`: повторный визит увеличивает `visitCount`/`totalSpentKopecks`, а не создаёт новую строку. Телефон хранится только как одно-направленный HMAC-SHA256-хэш (`GUEST_PHONE_HASH_PEPPER`) — plaintext нигде не сохраняется и не возвращается. `404 PAYMENT_INTENT_NOT_FOUND` |
| `DELETE /v1/public/payment-intents/:id/loyalty-optin` (B3) | `{ phone }` → `200 { deleted: boolean }` (`false` при повторном вызове — идемпотентно, не ошибка). Прагматичный MVP-флоу: гость доказывает владение номером повторным вводом того же телефона, **это не полноценная GDPR-верификация личности** (нет OTP/SMS-подтверждения) — знать чужой номер и ресторан, где он был, достаточно для удаления его профиля. `404 PAYMENT_INTENT_NOT_FOUND` |
...
| `GET /v1/staff/venues/:id/analytics/daily?date=YYYY-MM-DD` (B3) | owner/manager | — | `200 DailyAnalyticsResponse` — см. ниже. `400 INVALID_DATE` для невалидной даты; `404 VENUE_NOT_FOUND` для чужого tenant; `403 FORBIDDEN` для waiter |
...
`DailyAnalyticsResponse` (B3): `{ date, venueId, revenueKopecks, tipsKopecks, averageCheckKopecks, closedTablesCount, popularItems: PopularItem[], staffPerformance: StaffPerformanceEntry[] }`. Всё пересчитывается на каждый запрос из `payment_intents`/`order_items`/`orders` — нет materialized rollup. `date` — фиксированный UTC-календарный день (00:00:00.000Z .. 23:59:59.999Z), **не** локальный часовой пояс venue — документированное упрощение MVP. `PopularItem`: `{ menuItemId, name, quantitySold, revenueKopecks }`, топ-5 по `quantitySold`; удалённый пункт меню группируется по `name` (`menuItemId: null`), а не сливается в один бакет. `StaffPerformanceEntry`: `{ staffId, staffName, ordersOpened, revenueKopecks }`; `staffId: null, staffName: "Unattributed"` — бакет для заказов без `openedByStaffId` (созданных до B3-миграции), никогда не выбрасывается из суммы.

## Realtime (B3, WebSocket)

`GET /v1/staff/realtime` — WebSocket, не REST. Браузер не может выставить кастомный заголовок на хэндшейке, поэтому аутентификация — query-параметр `?token=<sessionToken>` (тот же самый bearer-токен из `Auth`, резолвится через тот же Redis session store). Codex: добавь `?token=<sessionToken>` к `NEXT_PUBLIC_STAFF_WS_URL` перед открытием соединения — единственное изменение, нужное на клиенте, контракт сообщений (`StaffRealtimeEvent`/`StaffRealtimeCommand`) не менялся с B2.5 (`packages/contracts/src/staffRealtime.ts`).

- Невалидный/просроченный токен → сервер закрывает соединение с кодом `4401` сразу после хэндшейка.
- Owner (`venueId: null` в сессии) подписан на все venue организации; manager/waiter — только на свой.
- При подключении сервер сразу шлёт **catch-up burst**: все `pending`/`seen` события за последние 24 часа для видимых venue, от старых к новым, каждое как `{ type: "staff.event", event: StaffRealtimeEvent }`. Это, а не отдельная retry/DLQ-таблица, делает доставку надёжной поперёк разрывов соединения — см. `apps/api/src/realtime/pubsub.ts`.
- Далее — живые события по мере наступления. Сервер шлёт `{ type: "ping" }` каждые 30с (keepalive, отвечать не нужно).
- Клиент может слать команды: `{ type: "notification.seen" \| "notification.resolved" \| "notification.archived", notificationId }`. Идемпотентны (безопасно повторить), forward-only (`pending → seen → resolved/archived`, откат/повтор — тихий no-op), а `notificationId` вне scope этого соединения (чужой venue/tenant) — тоже тихо игнорируется, не ошибка и не разрыв соединения.
- Типы событий: `bill_requested`, `payment_started`, `payment_succeeded`, `payment_failed`, `order_updated`, ~~`table_attention`~~ — последний зарезервирован в контракте, но **не эмитится** ни одним эндпоинтом (нет "позвать официанта"), сознательно отложено, не забыто.
