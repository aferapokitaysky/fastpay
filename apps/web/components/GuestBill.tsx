...
export function GuestBill({ bill: initialBill, token }: { bill: PublicBillResponse; token: string }) {
  ...
  if (mode === "success" && paid) return <section className="screen success"><div className="success-icon" aria-hidden="true">✓</div><p className="eyebrow">ОПЛАТУ ПІДТВЕРДЖЕНО</p><h1>Дякуємо!<br />{bill.order && paid.foodKopecks < bill.order.outstandingFoodKopecks ? "Вашу частину сплачено." : "Все сплачено."}</h1><div className="success-amount"><Money amountKopecks={paid.foodKopecks + paid.tipKopecks} size="lg" /><small>{paid.tipKopecks ? "Включно з чайовими" : "Оплату передано закладу"}</small></div><p className="receipt"><span>{bill.venue.name} · Стіл {bill.table.label}</span><span suppressHydrationWarning>{new Intl.DateTimeFormat("uk-UA", { hour: "2-digit", minute: "2-digit" }).format(paid.at)}</span></p><div className="loyalty"><span aria-hidden="true">✦</span><p><b>Повернемо 5% наступного разу</b><small>Залиште номер після візиту</small></p><span aria-hidden="true">›</span></div><button className="secondary full" onClick={() => setMode("bill")}>Переглянути рахунок</button></section>;
  if (mode === "processing") return <section className="screen processing"><div className="processing-orbit"><div className="spinner" aria-hidden="true" /><span>R</span></div><p className="eyebrow">БАНК ПІДТВЕРДЖУЄ ОПЛАТУ</p><h1>Ще мить…</h1><p>Не закривайте сторінку. Ми самі оновимо статус, щойно банк відповість.</p><div className="processing-note"><span>✓</span> Сума зафіксована, повторно платити не потрібно</div></section>;
  if (mode === "checkout") return <section className="screen checkout"><button className="back" onClick={() => setMode(checkoutFrom)} aria-label="Назад до рахунку">←</button><div className="checkout-step"><span>2</span><i /> <span>Підтвердити</span></div><p className="eyebrow">БЕЗПЕЧНА ОПЛАТА</p><h1>Перевірте суму</h1><p className="subtitle">{bill.venue.name} · Стіл {bill.table.label}</p><div className="checkout-total"><span>До сплати</span><Money amountKopecks={total} size="lg" /><small>{checkoutFrom === "split" ? "Ваша частина спільного рахунку" : "Повний рахунок за столом"}</small></div><div className="checkout-lines"><div><span>Страви</span><Money amountKopecks={food} /></div><div><span>Чайові</span><Money amountKopecks={tipAmount} /></div></div><div className="payment-method"><span className="card-brand">••••</span><p><b>Картка, Apple Pay або Google Pay</b><small>Спосіб оплати оберете на захищеній сторінці банку</small></p><span>›</span></div><div className="checkout-safe"><span>✓</span> Дані картки не передаються ресторану</div>{paymentError&&<p className="form-error" role="alert">{paymentError}</p>}<button className="primary full" onClick={confirmPayment}>Перейти до оплати <span>→</span></button></section>;

  return <section className={isSplit ? "screen split" : "screen"}>
    {isSplit ? <><button className="back" onClick={() => setMode("bill")} aria-label="Назад">←</button><p className="eyebrow">СТІЛ {bill.table.label} · {bill.venue.name}</p><h1>Що оплачуєте ви?</h1><p className="subtitle">Оберіть позиції зі спільного рахунку</p></> : <><div className="venue"><div className="venue-logo">{bill.venue.name[0]}</div><div><p className="eyebrow">ВАШ РАХУНОК</p><h1>{bill.venue.name}</h1></div><span className="table">Стіл {bill.table.label}</span></div><div className="guest-meta"><p className="live" role="status">Рахунок оновлено щойно</p><span>Захищено Rimvo</span></div><section className="guest-bill-summary" aria-label="Сума рахунку"><span>До сплати зараз</span><Money amountKopecks={food} size="lg" /><small>{payableIds.length} {payableIds.length === 1 ? "позиція" : "позиції"} у рахунку</small></section></>}
