...
## 2026-08-20 — Claude (B3 done: realtime delivery + reporting + loyalty)

- Added: `GET /v1/staff/realtime` (WebSocket, `@fastify/websocket`) — реализует контракт из COM-006 поле в поле. Query-param auth (`?token=`), owner/manager/waiter venue-scoping, catch-up burst (`pending`/`seen` события за 24ч) на подключении, 30с keepalive ping, идемпотентные forward-only команды (`notification.seen/resolved/archived`) с тихим игнором чужого tenant. Outbox-паттерн: `realtime_events` — durable-запись внутри той же транзакции, что и мутация; publish в in-process pub/sub (`realtime/pubsub.ts`) — только после коммита. Подключено в пять точек: `request-bill` → `bill_requested`, `PATCH` заказа → `order_updated`, создание payment intent → `payment_started`, webhook succeeded/failed → `payment_succeeded`/`payment_failed`. `table_attention` сознательно не эмитится — нет "позвать официанта" эндпоинта, это отложено, не забыто.
- Added: `GET /v1/staff/venues/:id/analytics/daily` — выручка/чаевые/средний чек/закрытые столы/топ-5 позиций/атрибуция по официантам, всё пересчитывается на лету из `payment_intents`/`order_items`/`orders`. `orders.openedByStaffId` — новая колонка (миграция `0003`), проставляется при открытии заказа; заказы без атрибуции (до миграции) не выбрасываются из сумм, а попадают в отдельный бакет `"Unattributed"`.
- Added: `POST/DELETE /v1/public/payment-intents/:id/loyalty-optin` — голосовой phone-only opt-in/out на экране "дякуємо" после оплаты. Апсерт `guest_profiles` по хэшу телефона (HMAC-SHA256, `GUEST_PHONE_HASH_PEPPER`, необратимо — в отличие от AES-шифрования платёжных данных из B2). Гейтится статусом intent (`succeeded` only), никогда не возвращает телефон/хэш. Opt-out — прагматичный MVP-флоу (повторный ввод телефона), явно не GDPR-верификация личности.
- Точные формы всех эндпоинтов — `docs/api/FRONTEND_BACKEND_CONTRACT.md` (разделы Realtime/Staff/Public), см. также COM-010 для WS auth handshake — единственное изменение, нужное на клиенте.
- Организационная заметка: снова начиналось с фонового агента; владелец второй раз (более резко) попросил не использовать агентов для реализации — остановил, доделал сам, руками, всё B3 целиком. Сохранил это как постоянное правило для себя на будущее.
- Verify: вручную прогнал весь цикл через реально запущенный dev-сервер (не только тестами) — открыл заказ (проверил `openedByStaffId`), PATCH/request-bill/payment-intent/webhook succeeded и failed с живым WebSocket-клиентом (`ws`) слушателем, подтвердил порядок и содержимое всех 5 типов событий, RBAC (waiter → `403` на analytics), auth-reject WS (`4401`), analytics-числа сошлись с ручным расчётом, loyalty gating/upsert/never-leaks-phone/idempotent-delete. Затем формальный сьют: 14 новых тестов (`test/realtimeAndReporting.test.ts`, WS через `app.injectWS()`, без реального network listener), полный набор — **87/87, дважды подряд**. `npm run lint/build -w apps/api` и `-w @fastpay/contracts` — чисто.
- Deferred (сознательно): `table_attention` (нет источника события), локальный часовой пояс venue в analytics (сейчас фиксированный UTC-день), materialized rollup для analytics (пересчёт на каждый запрос — ок при MVP-масштабе), горизонтальное масштабирование realtime (in-process `EventEmitter`, не Redis pub/sub — см. doc-comment в `realtime/pubsub.ts`).
- Next: реальная интеграция с эквайрингом (monobank/другой), когда появятся sandbox-ключи; либо следующий продуктовый приоритет владельца.
...
- По пути нашёл и починил реальный баг в уже смерженном коде: `publicBill.ts`/`orderState.ts` считали `remainingKopecks`/`outstandingFoodKopecks` как «всегда полная цена» — в B1 это было безобидно (никто не выставлял `payment_status: paid`), но теперь, когда вебхук реально это делает, без фикса оплаченные позиции продолжали бы показываться как неоплаченные.
- Организационная заметка: изначально это делал фоновый агент, но по ходу владелец попросил остановить агента и доделывать самому — так и сделал. Схема + `PaymentProvider`/`FakeProvider`/crypto, которые агент успел закоммитить/оставить незакоммиченными до остановки, оказались качественными (константное время сравнения подписи, случайный IV на шифрование), оставил как есть и продолжил поверх.
- Verify: вручную прогнал весь цикл через реальный запущенный сервер (не только тестами) — happy path (intent → подписанный webhook → `bill` показывает `paid`, `outstandingFoodKopecks: 0`), идемпотентность по ключу, гонка за одну и ту же позицию (и через `itemIds:"all"`, и через явный id), повторная доставка webhook, неверная подпись. Затем формальный сьют: 18 новых тестов, полный набор (8 файлов) — **74/74, дважды подряд без сброса БД**. `npm run lint/build -w apps/api` — чисто.
- Deferred (сознательно): реальный monobank/другой адаптер (нет sandbox-ключей), B3 (realtime-доставка, PaymentIntent-события в staff inbox), возвраты (только "internal recording" в проде понадобится отдельно), partial-item-amount split (фича-флаг вне MVP), фоновый воркер очистки зависших резерваций (expiry ленивый — сам снимается при следующей попытке, поведение задокументировано как осознанное упрощение).
- Next: B3 (realtime-доставка событий, включая `payment_succeeded`/`payment_failed`/`bill_requested` через уже формализованный контракт из COM-006) — либо реальная интеграция с эквайрингом, если у владельца появятся sandbox-ключи раньше.
